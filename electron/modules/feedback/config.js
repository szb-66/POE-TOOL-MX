export const FEEDBACK_CLOUDBASE_CONFIG = Object.freeze({
  ...
  dailyUsageTable: process.env.CLOUDBASE_DAILY_USAGE_TABLE || 'app_daily_usage_report',
...
export function isDailyUsageConfigured(config = FEEDBACK_CLOUDBASE_CONFIG) {
  return Boolean(config.envId && config.region && config.dailyUsageTable && config.publishableKey)
}
