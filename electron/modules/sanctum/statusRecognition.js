...
export function parseStatusTooltip(evidence, catalog) {
  ...
  for (let i = 0; i < texts.length; i++) {
    ...
    if (!rewardPrefix.test(text)) { effectTexts.push(texts[i]); continue }
    const raw = [texts[i]]
  ...
  const effectGroup = effectTexts.length || !rewardTexts.length ? parseEffectTooltip({...evidence,texts:effectTexts},catalog) : null
  ...
  return { effectGroup, rewardGroup,
