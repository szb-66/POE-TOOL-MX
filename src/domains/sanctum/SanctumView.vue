      ...
      <el-tab-pane label="楼层规划" name="floor">
        ...
        <div class="floor-layout">
          ...
          <aside><el-card shadow="never"><template #header>推荐路线</template>
            <el-alert v-if="store.state.savedRoute" :title="`${sanctumSavedLabel(store.state.savedRoute.savedAt)}；${store.state.savedRoute.incomplete ? '状态识别不完整，路线按当时已识别信息评估' : '再次采集可更新路线'}`" type="info" :closable="false" show-icon />
            ...
            <details v-if="readIssues.length" open><summary>读取失败与缺失项（{{ readIssues.length }}）</summary><p v-for="issue in readIssues" :key="effectIssueText(issue)">{{ effectIssueText(issue) }}</p><el-button size="small" :disabled="store.readOnly || store.busy || !identity" @click="perform('rescanEffects')">重读状态栏</el-button></details>
        ...
        <SanctumRunObservation :state="store.state" :disabled="store.readOnly || store.busy || !identity" :action="perform" />
        <el-drawer :model-value="Boolean(selectedRoom)" title="房间详情" size="min(720px, 100vw)" append-to-body @close="selectedId = ''"><SanctumRoomDetails v-if="selectedRoom" :key="`${floor.runId}:${floor.floorId}:${selectedId}`" :room="selectedRoom" /></el-drawer>
      </el-tab-pane>
...
import { captureButton, roomTitle, calibrationIssues, sanctumPageDisplay, sanctumSavedLabel } from '../../../shared/sanctumPresentation.js'
...
const floor = computed(() => store.state.savedRoute?.floor || store.state.floor), identity = computed(() => store.state.floor?.identityConfirmed === true)
...
const shownRecommendation = computed(() => store.state.savedRoute?.recommendation || store.state.recommendation)
const display = computed(() => sanctumPageDisplay(floor.value, shownRecommendation.value, store.state.savedRoute?.marks || store.state.marks, store.state.savedRoute ? null : store.state.progress))
