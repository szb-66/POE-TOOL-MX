...
<section class="sanctum-section">
          <el-card  shadow="never">
            ...
            <SanctumRoomRecognition v-if="evidence" :room="selectedRoom" :floor="store.state.savedRoute?.floor || store.state.floor" />
            <div v-if="editing">
            <el-form :inline="true" label-position="top" :disabled="store.readOnly || store.busy || !store.state.floor?.identityConfirmed">
              ...
              <el-form-item label="房间类型"><el-select v-model="roomDraft.type" placeholder="待确认" clearable style="width: 160px" :disabled="store.readOnly || store.busy || !store.state.floor?.identityConfirmed"><el-option v-for="(label, value) in roomTypeLabels" :key="value" :label="label" :value="value" /></el-select></el-form-item>
              <el-form-item label="战斗布局"><el-select v-model="roomDraft.layout" placeholder="待确认" clearable style="width:150px"><el-option v-for="(label,key) in layoutLabels" :key="key" :label="label" :value="key" /></el-select></el-form-item>
