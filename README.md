...
在首页选择“导出诊断”。导出的 JSON 仅包含应用版本、Windows 与架构、显示器布局、DPI、运行时版本和模块状态。生成阶段会排除 POESESSID、账号信息和完整用户路径；提交前仍建议自行快速检查内容。

## 数据与隐私

- 账号会话信息仅保存在本机 Electron 独立 Session 或本地配置中，不会写入诊断文件。
