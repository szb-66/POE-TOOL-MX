...
系统 SHALL 根据本次游戏实测生成数值型自动化时序建议，并 MUST 明确区分直接实测字段和保守推导字段；建议不得包含已删除的自适应配置。
...
- **THEN** `buttonHoldMs` 取当前值、默认 15ms 和推荐 `keyHoldMs` 的最大值
- **AND** `releaseSettleMs` 取当前值、默认 10ms 和键盘释放实测安全值的最大值
- **AND** `stashTabSettleMs` 取当前值、默认 10ms 和综合响应信号两倍的最大值
- **AND** `stashSettleMs` 取当前值、默认 10ms 和综合响应信号两倍的最大值
- **AND** `patchVerifyMs` 取当前值、默认 10ms 和综合响应信号三倍的最大值
- **AND** 上述字段标记为推导且不得低于用户当前值或现有安全默认值
- **AND** 系统不生成、展示或返回 `adaptiveTiming` 或 `adaptiveTimeoutMs` 建议
...
#### Scenario: 保留自适应开关
- **WHEN** 旧设置包含自适应开关或自适应等待上限
- **THEN** 自动检测忽略这些值且不保留、推荐或修改它们
