...
系统 MUST 使用官方国服物品目录按底材解析所有未鉴定传奇，包括传奇药剂，并 MUST 在身份未解决时禁止发送官方搜索。身份解决后系统 MUST 保留原始物品大类和未鉴定过滤。

#### Scenario: 唯一候选
- **WHEN** 未鉴定传奇底材只对应一个去重后的传奇名称
- **THEN** 系统自动应用该名称和底材并保留未鉴定过滤后查询

#### Scenario: 未鉴定传奇药剂唯一候选
- **WHEN** 未鉴定传奇药剂底材只对应一个去重后的传奇名称
- **THEN** 系统自动应用该名称和药剂底材，并以药剂大类及未鉴定过滤查询

#### Scenario: 多个候选
- **WHEN** 同一底材对应多个传奇名称
- **THEN** 浮窗显示候选并等待用户选择，选择前不得发送搜索

#### Scenario: 未鉴定传奇药剂多个候选
- **WHEN** 同一药剂底材对应多个传奇名称
- **THEN** 浮窗使用通用传奇候选交互等待用户选择，选择后仍保留药剂大类和未鉴定过滤
