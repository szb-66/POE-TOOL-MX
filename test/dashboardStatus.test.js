...
test('首页使用单一紧凑汇总带并以文字和图标表达模块状态', () => {
  ...
  assert.match(dashboard, /v-if="item\.action"/)
  assert.match(dashboard, /@click="openHealthAction\(item\.action\)"/)
  assert.match(dashboard, /class="health-content"/)
  assert.match(dashboard, /class="health-action"/)
