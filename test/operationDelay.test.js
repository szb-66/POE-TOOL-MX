...
test('制作和地图脚本的悬停等待来自同一个真实值，点击与复制使用内部固定时序', () => {
  ...
  for (const template of [
    ...
    assert.match(template, /timeout_seconds = ADAPTIVE_TIMEOUT_SECONDS if TIMING_MODE == "adaptive" else CLIPBOARD_RESPONSE_MIN_SECONDS/)
    ...
    assert.match(template, /wait_for_clipboard_change\(before_seq, before_text, timeout_seconds, allow_unchanged_text\)/)
