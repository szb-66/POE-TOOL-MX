...
const primaryPages = [
  ...
  'src/domains/sanctum/SanctumView.vue',
  'src/domains/tools/ToolsView.vue',
  'src/domains/pobExport/PobExportView.vue',
...
test('主布局和导航保持 76px、原顺序、预加载与非交互视觉分组', () => {
  ...
  assert.deepEqual(availableNavigationFeatureCatalog({ development: true }).map(item => [item.route, item.label]), [
    ...
    ['/craft-planner', '模拟'], ['/price-check', '查价'], ['/faustus', '浮士德'], ['/puzzle', '海图'], ['/sanctum', '圣所'], ['/tools', '工具站'], ['/pob-export', 'POB']
  ])
