...
test('圣所页面及运行时通过 Vite 开发转换，正式导航包含圣所', async () => {
  ...
  try {
    ...
    for (const name of ['SanctumCalibrationCollection', 'SanctumEffectRules', 'SanctumEffectReview', 'SanctumRecognitionImage', 'SanctumMapGraph', 'SanctumMapLegend', 'SanctumStrategy', 'SanctumRecognitionCalibration', 'SanctumRoomDetails', 'SanctumRunObservation', 'SanctumView']) {
    ...
    for (const url of ['/src/domains/sanctum/SanctumRunObservation.vue', '/src/domains/settings/CoordinatePickerView.vue', '/src/domains/sanctum/SanctumView.vue', '/src/domains/sanctum/SanctumLiveCalibration.vue', '/src/domains/sanctum/SanctumOverlayView.vue', '/src/domains/sanctum/SanctumControlOverlayView.vue', '/src/domains/sanctum/SanctumCalibration.vue', '/src/domains/sanctum/sanctumStore.js',
