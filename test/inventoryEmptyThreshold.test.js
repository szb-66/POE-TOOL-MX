...
test('设置页持久化共享阈值并支持旧值补全和重置', () => {
  ...
  const defaults = source('../src/utils/environmentDefaults.js')
  ...
  assert.match(defaults, /emptySlotThreshold: EMPTY_SLOT_THRESHOLD\.default/)
  assert.match(store, /createDefaultInventorySettings\(\)/)
