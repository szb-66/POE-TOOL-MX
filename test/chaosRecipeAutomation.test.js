...
test('配方取件每次移动、复制和点击前检查前台，失焦时释放输入', () => {
  ...
  assert.match(source, /self\.clipboard_timeout = max\(CLIPBOARD_RESPONSE_MIN_SECONDS, self\.delay \* 4\)/)
  assert.match(source, /deadline = time\.monotonic\(\) \+ self\.clipboard_timeout/)
  assert.match(source, /InputController\(config\.get\("operation_delay_ms", 80\)\)/)
  assert.doesNotMatch(source, /timing_mode|adaptive_timeout_ms/)
